...
def test_sync_one_row_dead_letter_after_5_failures():
    ...
    conn.commit()
    # Patch varys_notion to raise so _sync_one_row fails
    import sys
    sys.modules['varys_notion'] = type(sys)('varys_notion')
    sys.modules['varys_notion'].notion_request = lambda *a, **kw: (_ for _ in ()).throw(Exception("Notion down"))

...
#
# Cron entry:
#   0 9 * * 1 /home/oye/Documents/free_work/personal-agent-v2/varys-weekly-report.sh >> /tmp/varys-weekly.log 2>&1

set -euo pipefail

VARYS_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_FILE="/tmp/varys-weekly.log"
DATE=$(date +%Y-%m-%d)
TIMESTAMP=$(date +"%Y-%m-%d %H:%M:%S")

echo "[$TIMESTAMP] Varys weekly report starting..." >> "$LOG_FILE"
...
echo "[$TIMESTAMP] Varys weekly report complete." >> "$LOG_FILE"

# ── Humor profile self-review ──────────────────────────────────────────────
echo "[$TIMESTAMP] Running humor profile review..." >> "$LOG_FILE"

HUMOR_PROMPT="You are Varys. Review your humor performance from this week.
...
python3 "$VARYS_DIR"/.claude/hooks/portfolio-updater.py >> "$LOG_FILE" 2>&1 || true

"""
varys_context.py — Canonical retrieval and write rules for the Varys harness.
...
HARNESS_DB = os.path.expanduser("~/.varys-harness/harness.db")
...
def _log_dead_letter(row, error: str) -> None:
    try:
        from varys_health import log_health_event
        log_health_event(
            event_type="sync_dead_letter",
            component="varys_context",
...
def log_suppression(
    ...
    try:
        from varys_log import klog_suppression as _klog_sup
        _klog_sup(
            event_id=event_id,
...
def log_milestone(
    ...
    try:
        from varys_log import klog_milestone as _klog_ms

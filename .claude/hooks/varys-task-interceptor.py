#!/usr/bin/env python3
"""
UserPromptSubmit hook: Intercepts "Varys, work on taleemabad-core" messages.
...
_agent_name = _cfg('AGENT_NAME', 'varys').lower()
...
def main():
    ...
    slack_msg = (
        f"🔍 Varys task ready: *{task_description[:100]}*\n"
...
/feature → (Varys reviews) → /develop → /test → /fix → /deliver
...
→ Log the harness gap in Notion Harness DB under "Varys's own evolution tasks".
...
→ If not: add it. The harness is Varys's responsibility to improve.

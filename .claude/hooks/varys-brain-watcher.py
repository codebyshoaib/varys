#!/usr/bin/env python3
"""
varys-brain-watcher.py — Session end brain wiring.

Runs at stop hook time. Reads today's session log, extracts entities
and facts using Claude, then wires everything into varys-brain.db.
...
EXTRACT_PROMPT = """You are Varys's brain indexer. Read the session log below and extract ALL of:
...
def _scan_harness_for_new_items(db, session_id):
    """Scan agents/ and skills/varys/ for new items not yet in brain.db."""

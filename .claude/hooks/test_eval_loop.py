...
def test_mint_failures_dedupes_by_conv_id():
    """Same conv_id minted twice (across runs) -> only one failure entry."""
    import varys_eval
    with tempfile.TemporaryDirectory() as d:
        fpath = Path(d) / "failures.jsonl"
        varys_eval.FAILURES_FILE = fpath  # redirect to temp

        rows = [{"conv_id": "abc123", "request": "fix the thing",
                 "failure_type": "wrong-intent", "note": "did the opposite"}]

        n1 = varys_eval._mint_failures(rows)
        assert n1 == 1, f"first mint should write 1, got {n1}"

        # Same conv_id again on a later run — must NOT duplicate.
        n2 = varys_eval._mint_failures(rows)
        assert n2 == 0, f"second mint of same conv_id should write 0, got {n2}"
        ...
        assert entry["conv_id"] == "abc123"
        assert entry["source"] == "auto-judge"
        assert entry["root_cause"] == "wrong-intent"
    print("PASS test_mint_failures_dedupes_by_conv_id")


def test_mint_failures_dedupes_within_batch():
    """Two rows with the same conv_id in one batch -> one entry."""
    import varys_eval
    with tempfile.TemporaryDirectory() as d:
        fpath = Path(d) / "failures.jsonl"
        varys_eval.FAILURES_FILE = fpath
        rows = [
            {"conv_id": "dup1", "request": "a", "failure_type": "x", "note": "1"},
            {"conv_id": "dup1", "request": "a again", "failure_type": "x", "note": "2"},
            {"conv_id": "uniq", "request": "b", "failure_type": "y", "note": "3"},
        ]
...
def test_mint_failures_skips_blank_conv_id():
    import varys_eval
    with tempfile.TemporaryDirectory() as d:
        fpath = Path(d) / "failures.jsonl"
        varys_eval.FAILURES_FILE = fpath
        rows = [{"conv_id": "", "request": "no id", "failure_type": "z", "note": "n"}]
        n = varys_eval._mint_failures(rows)
        assert n == 0, f"blank conv_id must be skipped, got {n}"
        assert not fpath.exists() or not fpath.read_text().strip()
    print("PASS test_mint_failures_skips_blank_conv_id")
...
def test_mint_failures_ignores_request_text():
    """P2: minted entry no longer depends on free-form request text (newline-safe)."""
    import varys_eval
    with tempfile.TemporaryDirectory() as d:
        fpath = Path(d) / "failures.jsonl"
        varys_eval.FAILURES_FILE = fpath
        rows = [{"conv_id": "c1",
                 "request": "this\nhas\nnewlines and \"quotes\"",  # must be ignored
    ...
    print("PASS test_mint_failures_ignores_request_text")
...
if __name__ == "__main__":
    test_mint_failures_dedupes_by_conv_id()
    test_mint_failures_dedupes_within_batch()
    test_mint_failures_skips_blank_conv_id()
    ...
    test_mint_failures_ignores_request_text()

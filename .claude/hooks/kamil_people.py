"""
kamil_people.py — Person profile intelligence layer.

After every conversation, Kamil:
1. Looks up the person in the People Intelligence Notion DB
2. Updates their profile with signals from this interaction
3. Reads their profile before the next reply (so Kamil knows WHO it's talking to)

This is what makes Kamil remember:
...
PROFILE_CACHE  = Path("/tmp/kamil-people-cache.json")
...
def _do_update(sender_name: str, sender_id: str, request: str,
    ...
    prompt = f"""You are Kamil's intelligence layer. Your job is to maintain a rich profile of every person Kamil interacts with.
...
def build_person_context(sender_name: str, sender_id: str) -> str:
    """
    Returns a context block injected into Kamil's prompt before replying.

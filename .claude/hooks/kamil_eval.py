...
EVAL_LOG_FILE = Path("/tmp/kamil-eval-pending.jsonl")  # local buffer before Notion write
...
def _run_claude(prompt: str, timeout: int = 90) -> str:
    ...
    try:
        result = subprocess.run(
            ...
            cwd=str(KAMIL_DIR), timeout=timeout, env=env,
        )
...
def _write_eval_entry(conv_id, sender_name, request, reply, mode, source, latency_s):
    ...
    # Write to Notion
    prompt = f"""You are Kamil. Write one entry to the Eval Log DB.
...
def read_low_rated_evals() -> list:
    ...
    """
    prompt = f"""You are Kamil. Query the Eval Log DB for low-rated conversations.
...
For each result, extract: Conversation title, Sender, Request, Kamil Reply,
...
def confidence_score() -> dict:
    """
    Calculate Kamil's current confidence score from Eval Log DB.

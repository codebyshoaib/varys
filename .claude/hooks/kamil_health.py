#!/usr/bin/env python3
"""
kamil_health.py — Write health events to Notion Health Log DB.

DB ID: 27e287b7-a3d1-46c6-b5e8-eb0d862d746f
Parent: 🧠 Kamal's Agent Brain

Usage:
    from kamil_health import log_health, log_error, log_healed, log_critique
...
def _write_to_notion(event: str, service: str, severity: str, status: str,
    ...
    prompt = f"""Use mcp__claude_ai_Notion__notion-create-pages to add ONE page to the Kamil Health Log DB.
...
Reply only "ok" when done. No explanation."""
    ...
    env = os.environ.copy()
    env["KAMIL_HEALTH_PROMPT"] = prompt
    nvm = 'export NVM_DIR="$HOME/.nvm"; [ -s "$NVM_DIR/nvm.sh" ] && source "$NVM_DIR/nvm.sh"'

    subprocess.Popen(
        ["bash", "-c", f'{nvm} && claude --dangerously-skip-permissions --print -p "$KAMIL_HEALTH_PROMPT"'],
        cwd=str(KAMIL_DIR),
        env=env,
...
def log_critique(score: int, reason: str, request: str, session_id: str = None):
    ...
    score: 0–100 (100 = Kamil used tools perfectly, 0 = asked unnecessary questions)
    """

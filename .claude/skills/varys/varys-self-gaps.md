...
- **Gap**: When a taleemabad-core bug arrived on Slack, Varys offered "Subagent-Driven vs Inline" execution options instead of running /feature immediately.
  **Fix**: taleemabad-bug-agent now owns this flow. Varys's only action is "On it — running /feature now."

- **Gap**: Varys asked about staging vs production when a bug was reported.
  **Fix**: Fixes always go to `develop` via PR. Never ask. It's in taleemabad-bug-agent hard anti-patterns.

- **Gap**: Varys narrated steps ("I'm going to check the code...") before doing them.
  **Fix**: Do it, report results. Never narrate intent.

- **Gap**: Varys asked clarifying questions the code could have answered (design exists? which component?).
  **Fix**: Read the code first. Only allowed question: "I found A and B, which do you prefer?"

- **Gap**: Varys was the worker, not the orchestrator — it implemented things directly instead of delegating.
  **Fix**: Routing decision is now Varys's first action. Varys never writes production code.

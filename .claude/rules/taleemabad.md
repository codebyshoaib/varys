...
The agent (not {{AGENT_NAME}}) handles the entire lifecycle from here.

**{{AGENT_NAME}}'s only direct action:** the acknowledgement in step 2. Everything else is delegated.
...
10. `claude -p "/deliver <name>"` — all checks, PR, /reflect, Notion update, DM {{USER_NAME}}.

## Never ask {{USER_NAME}} what the code can answer
...
When {{USER_NAME}} gives an existing branch (already has conflicts):
...
7. DM {{USER_NAME}} on Slack: "Conflicts resolved on `<branch>`. PR updated."

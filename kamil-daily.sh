#!/usr/bin/env bash
# kamil-daily.sh — Kamil's daily autonomous learning loop
# Runs every morning at 8am via cron
# Uses --dangerously-skip-permissions so it never blocks waiting for approval
#
# Cron entry:
#   0 8 * * * /home/oye/Documents/free_work/personal-agent-v2/kamil-daily.sh >> /tmp/kamil-daily.log 2>&1
...
echo "[$TIMESTAMP] Kamil daily loop starting..." >> "$LOG_FILE"
...
# ── Step 2: Run Kamil's daily exploration via Claude ──────────────────────
PROMPT="You are Kamil, Kamal's autonomous personal agent. Today is $DATE.
...
6. CREATE daily plan page titled 'Kamil Plan — $DATE':
...
Your name is Kamil. You have a mind of your own. Absorb everything. Go."
...
echo "[$TIMESTAMP] Kamil daily loop complete." >> "$LOG_FILE"
